terraform{
    required_providers {
        nutanix = {
            source = "nutanix/nutanix"
            version = "2.0"
        }
    }
}

#defining nutanix configuration
provider "nutanix"{
  username = var.nutanix_username
  password = var.nutanix_password
  endpoint = var.nutanix_endpoint
  port = 9440
  insecure = true
}

# create a subnet
resource "nutanix_subnet_v2" "example" {
  name              = "subnet_for_route"
  description       = "subnet to test create route"
  cluster_reference = "<cluster_uuid>"
  subnet_type       = "VLAN"
  network_id        = "<vlan_uuid>"
  is_external       = true
  ip_config {
    ipv4 {
      ip_subnet {
        ip {
          value = "10.44.3.192"
        }
        prefix_length = "27"
      }
      default_gateway_ip {
        value = "10.44.3.193"
      }
      pool_list {
        start_ip {
          value = "10.44.3.198"
        }
        end_ip {
          value = "10.44.3.207"
        }
      }
    }
  }
}

# crete a vpc
resource "nutanix_vpc_v2" "example" {
  name        = "terraform_test_vpc_1"
  description = "terraform test vpc 1 to test create route"
  external_subnets {
    subnet_reference = nutanix_subnet_v2.example.id
  }
  depends_on = [nutanix_subnet_v2.example]
}

# create a vpc
resource "nutanix_vpc_v2" "example" {
  name        = "terraform_test_vpc"
  description = "terraform test vpc to test create route"
  external_subnets {
    subnet_reference = nutanix_subnet_v2.example.id
  }
  depends_on = [nutanix_subnet_v2.example]
}

# get route table
data "nutanix_route_tables_v2" "example" {
  filter     = "vpcReference eq '${nutanix_vpc_v2.example.id}'"
  depends_on = [nutanix_vpc_v2.example]
}

# create a route
resource "nutanix_routes_v2" "test-1" {
  name               = "terraform_test_route"
  description        = "terraform test route to test create route"
  vpc_reference      = nutanix_vpc_v2.example.id
  route_table_ext_id = data.nutanix_route_tables_v2.example.route_tables[0].ext_id
  destination {
    ipv4 {
      ip {
        value = "10.0.0.2"
      }
      prefix_length = 32
    }
  }
  next_hop {
    next_hop_type      = "EXTERNAL_SUBNET"
    next_hop_reference = nutanix_subnet_v2.example.id
  }
  metadata {
    owner_reference_id   = nutanix_vpc_v2.example.id
    project_reference_id = "<project_uuid>"
  }
  route_type = "STATIC"
}

# list all routes
data "nutanix_routes_v2" "all-routes" {
  route_table_ext_id = "<route_table_uuid>"
}

# list all routes with filter
data "nutanix_routes_v2" "filtered-routes" {
  route_table_ext_id = "<route_table_uuid>"
  filter = "vpcReference eq '<vpc_uuid>'"
}

# list all routes with limit
data "nutanix_routes_v2" "limited-routes" {
  route_table_ext_id = "<route_table_uuid>"
  limit = 3
}

# fetch route by id
data "nutanix_route_v2" "route-by-id" {
  route_table_ext_id = "<route_table_uuid>"
  ext_id = "<route_uuid>"
}