...
var (
	configFilePath = cfg.ConfigPath + "/virtualmachineconfig.autogenerated.go"
	schemaFilePath = cfg.SchemaPath + "/virtualmachineschema.autogenerated.go"
...
func xreflect(name string) ([]byte, []byte, error) {
	...
	file ,err := os.Open(os.ExpandEnv( cfg.StructPath + fromCamelcase(structNameMap[name]) +".go"))
	if err != nil {
		log.Fatal(err)
	}
    defer file.Close()
