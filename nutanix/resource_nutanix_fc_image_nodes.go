...
func resourceNutanixFCImageCluster() *schema.Resource {
	return &schema.Resource {
		...
		StateUpgraders: []schema.StateUpgrader{
			{
				Type:    resourceNutanixFCImageClusterInstanceResourceV0().CoreConfigSchema().ImpliedType(),
				Upgrade: resourceFCImageClusterInstanceStateUpgradeV0,
				Version: 0,
			},
		},
...
func resourceNutanixFCImageClusterCreate(d *schema.ResourceData, meta interface{}) error {
  ...
  client:= meta.(*Client).API
  uuid:= d.id()

  //Make request to the API
  resp, err := conn.fc.GetImage(uuid)
  if err != nil {
