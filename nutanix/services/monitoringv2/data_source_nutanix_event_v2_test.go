		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet(datasourceNameEvent, "metric_details.#"),
				),
			},
		},
	})
...
func testAccEventDataSourceConfig() string {
	return `
data "nutanix_events_v2" "list" {
  limit = 1
}

data "nutanix_event_v2" "test" {
  ext_id = data.nutanix_events_v2.list.events[0].ext_id
  depends_on = [data.nutanix_events_v2.list]
}
`
}
