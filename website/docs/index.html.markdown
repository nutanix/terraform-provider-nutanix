...
* `insecure` - (Optional) Set to `true` to skip TLS certificate verification. This can also be specified with `NUTANIX_INSECURE`. Defaults to `false`, meaning certificates are verified.

  **Note:** Prior to this release, the v4 (`*_v2`) resources and data sources ignored `insecure` and never verified certificates. They now honour it. If your Prism Central presents a self-signed or otherwise untrusted certificate, upgrading will produce `x509: certificate signed by unknown authority`. Install a trusted certificate, or set `insecure = true` to explicitly accept the risk — doing so exposes your credentials to man-in-the-middle attacks.
