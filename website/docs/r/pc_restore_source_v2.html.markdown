...
> **Authentication:** The `nutanix_pc_restore_source_v2` resource does **not** support API key authentication. The CreateRestoreSource API routes requests to the target cluster (Prism Element), which only supports basic auth (username/password). Use `username` and `password` in the provider configuration—do not use `api_key`. Using API key will result in "Invalid cookies present in the request" errors.
...
See detailed information in [Nutanix Restore Source V4](https://developers.nutanix.com/api-reference?namespace=prism&version=v4.4#tag/DomainManager/operation/createRestoreSource).
